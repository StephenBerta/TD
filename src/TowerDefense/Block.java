...
public class Block extends Rectangle {
	...
	public void physic() {
		...
		if(!shooting && airID == Value.airTowerLaser || airID == Value.airTowerRadiator || airID == Value.airTowerCannon) {
			for(int i=0;i<Screen.mobs.length;i++) {
				if(Screen.mobs[i].inGame && towerRange.intersects(Screen.mobs[i])) {
					shooting = true;
					shotMob = i;
				}
			}
		}
		if(shooting && Screen.mobs[shotMob].isDead()) {
			shooting = false;
			shotMob = -1;
			Screen.hasWon();
		}
	...
	public void fight(Graphics g) {
		if(Screen.isDebug && airID == Value.airTowerLaser) {
		...
		if(shooting && airID == Value.airTowerLaser) {
		...
		if(shooting && airID == Value.airTowerRadiator) {
			for(int i=0;i<Screen.mobs.length;i++) {
				if(towerRange.intersects(Screen.mobs[i]) && Screen.mobs[i].inGame) {
					...
					g.drawLine(
						x + (width / 2),
						y + (height / 2),
						Screen.mobs[i].x + (Screen.mobs[i].width / 2),
						Screen.mobs[i].y + (Screen.mobs[i].height / 2));
					if(loseFrame >= loseTime) {
		...
		if(shooting && airID == Value.airTowerCannon) {
			...
			for(int i = 0; i < 16; i++) {
				g.drawLine(
					x + (width / 2) - i / 4,
					y + (height / 2),
					Screen.mobs[shotMob].x + (Screen.mobs[shotMob].width / 2) + i ,
					Screen.mobs[shotMob].y + (Screen.mobs[shotMob].height / 2)
				);
			}
			if(loseFrame >= loseTime) {
